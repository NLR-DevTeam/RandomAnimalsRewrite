...
public class RandomFox extends JRawCommand {
    public RandomFox() {
        super(RandomAnimals.INSTANCE, "random-fox", "来只狐", "来只狐狸", "来只狐狐");
        setDescription("随机来张狐狸图");
    ...
    public void onCommand(@NotNull CommandSender sender, @NotNull MessageChain args) {
        ...
        if (Cooler.isLocked(uid)) {
            sender.getSubject().sendMessage("小狐狸跑累了，请待会再来！");
            return;
        }
        Cooler.lock(uid, PluginConfig.INSTANCE.coolDownTime.get());

        sender.getSubject().sendMessage("请稍等，狐狐正在跑步前进！");

        Future<?> task = RandomAnimals.downloadExecutorService.submit(() -> {
            try {
                ExternalResource externalResource = DownloadUtil.downloadAndParse("https://randomfox.ca/floof/", "image");
                ...
                sender.getSubject().sendMessage("小狐狸因为错误迷路了，请到控制台查看详细信息!\n" + ex);
                ex.printStackTrace();
            }
        });
        ...
        RandomAnimals.watchExecutorService.submit(() -> {
            ...
            task.cancel(true);
            sender.getSubject().sendMessage("狐狐图片获取超时，请稍后再试!");
        });
    }
}
