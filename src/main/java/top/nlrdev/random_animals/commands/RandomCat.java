...
public class RandomCat extends JRawCommand {
    public RandomCat() {
        super(RandomAnimals.INSTANCE, "random-cat", "来只猫", "来只猫猫", "来只修猫", "来只喵");
        setDescription("随机来张猫猫图");
    ...
    public void onCommand(@NotNull CommandSender sender, @NotNull MessageChain args) {
        ...
        if (Cooler.isLocked(uid)) {
            sender.getSubject().sendMessage("猫猫跑累了，请待会再来！");
            return;
        }
        Cooler.lock(uid, PluginConfig.INSTANCE.coolDownTime.get());

        sender.getSubject().sendMessage("请稍等，猫猫正在跑步前进！");

        Future<?> task = RandomAnimals.downloadExecutorService.submit(() -> {
            try {
                ExternalResource externalResource = DownloadUtil.downloadAndParse("https://api.thecatapi.com/v1/images/search", "url", json -> (JSONObject) new JSONArray(json).get(0));
                ...
                sender.getSubject().sendMessage("猫猫因为错误迷路了，请到控制台查看详细信息!\n" + ex);
                ex.printStackTrace();
            }
        });
        ...
        RandomAnimals.watchExecutorService.submit(() -> {
            ...
            task.cancel(true);
            sender.getSubject().sendMessage("猫猫图片获取超时，请稍后再试!");
        });
    }
}
